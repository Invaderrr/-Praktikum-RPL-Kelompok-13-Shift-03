        ...
        <nav class="flex-1 px-4 mt-4 space-y-2">
            ...
            <a href="{{ route('user.transaksi') }}" 
               ...
               style="background: linear-gradient(93.69deg, rgba(219, 212, 0, 0.5) 0.99%, rgba(245, 158, 11, 0.5) 100%);"> 
                <img alt="Transaksi Icon" class="w-5 h-5 mr-3 brightness-0" src="{{ asset('img/Dollar.png') }}"/>
                    ...
                    <tbody class="text-sm text-gray-700">
                        @forelse($riwayat as $t)
                            ...
                            <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-3">
                                    @if($index === 0)
                                        <div class="font-bold">{{ $t->created_at->format('d M Y') }}</div>
                                        <div class="text-[10px] text-gray-400">{{ $t->created_at->format('H.i') }} WIB</div>
                                    @endif
                                </td>
                                <td class="px-6 py-3">{{ $detail->nama_item }}</td>
